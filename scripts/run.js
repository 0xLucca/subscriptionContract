const main = async () => {
  ...
  let userEthBalance = await owner.getBalance();
  console.log(
    "User ETH balance before deploying: ",
  ...
  userEthBalance = await owner.getBalance();
  console.log(
    "User ETH balance after deploying:",
    ethers.utils.formatEther(userEthBalance)
  );

  let userSubscription = await subscriptionContract.getSubscriptionStatus(
    owner.getAddress()
  );
  console.log("User subscription status: ", userSubscription);

  let contractDaiBalance = await dai.balanceOf(subscriptionContract.address);
  console.log(
    "Subscription Contract DAI balance:",
    ethers.utils.formatEther(contractDaiBalance)
  );

  const options = { value: ethers.utils.parseEther("4.0") };
  // Call the function.
  let txn = await subscriptionContract.paySubscription(options);
  //console.log(txn);
  ...
  userSubscription = await subscriptionContract.getSubscriptionStatus(
    owner.getAddress()
  );
  console.log("User subscription status: ", userSubscription);

  contractDaiBalance = await dai.balanceOf(subscriptionContract.address);
  console.log(
    "Subscription Contract DAI balance:",
    ethers.utils.formatEther(contractDaiBalance)
  );
